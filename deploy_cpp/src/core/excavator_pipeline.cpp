...
struct PipelineState {
    ...
    bool pending_bucket_secured = false;
...
private:
    ...
    void trigger_bucket_count(long long now) {
        ...
        if (state.last_avg_ratio < 0.0f) {
            ...
            if (state.last_avg_ratio == 0.0f && avg_ratio == 0.0f) {
                ...
                float decline = (state.last_avg_ratio > 0.0f) ? (state.last_avg_ratio - avg_ratio) / state.last_avg_ratio : 0.0f;
...
public:
    ...
    void process(cv::Mat& frame) {
        ...
        if (!bucket_boxes.empty()) {
            ...
            if (best_bucket.class_id == 1) {
                ...
                if (!state.bucket_full) {
                    ...
                    bool is_digging_low = false;

                    if (main_truck_rect.area() > 0) {
                        bool horizontal_overlap = check_horizontal_overlap(bb_rect, main_truck_rect);
                    ...
                    if ((!overlap_main || is_digging_low) && !state.dumping_active && dumping_boxes.empty()) {
                        state.full_confirm_frames++;
                        if (state.full_confirm_frames >= 4) {
                ...
                if (state.bucket_full) {
                    ...
                    // 💥 融合了 C++ 原本的物理高度约束和 Python 的锁约束逻辑
                    if (state.pending_bucket_secured) {
                        is_valid_empty = true;
                    } else if (main_truck_rect.area() > 0) {
                        bool horizontal_overlap = check_horizontal_overlap(bb_rect, main_truck_rect);
                        bool scale_matched = (main_truck_rect.width > bb_rect.width * 1.5f);
                        bool overlap_main = horizontal_overlap && scale_matched;

                        bool is_hovering_high = true;
                        if (bb_rect.y > main_truck_rect.y + main_truck_rect.height * 0.4f) is_hovering_high = false;
                        if (bb_rect.y + bb_rect.height > main_truck_rect.y + main_truck_rect.height * 0.8f) is_hovering_high = false;

                        is_valid_empty = overlap_main && is_hovering_high;
                    }

                    if (is_valid_empty) {
                        state.empty_confirm_frames++;
                        // 维持 C++ 特调的 6 帧阈值
                        if (state.empty_confirm_frames >= 6) {
                            ...
                            if (!state.pending_bucket_secured) {
                                state.secured_dump_start_time = now - 2000;
                            }

                            // 💥 瞬间触发结账
                            trigger_bucket_count(now);
        ...
        if (has_dumping) {
            ...
            if (!state.dumping_active && state.dumping_frame_count >= 2) {
                ...
                if (!bucket_boxes.empty()) {
                    ...
                    auto d = dumping_boxes[0];
                    state.last_dumping_bucket_box = cv::Rect(d.xmin, d.ymin, d.xmax - d.xmin, d.ymax - d.ymin);
                }

                bool is_dumping_inside_truck = false;

                if (main_truck_rect.area() > 0) {
                    bool overlap_x = check_horizontal_overlap(state.last_dumping_bucket_box, main_truck_rect);
                    if (overlap_x) {
                        is_dumping_inside_truck = (state.last_dumping_bucket_box.y < main_truck_rect.y);
                    } else {
        ...
        // ============================== D. 实时跟踪采集核心 ==============================
        bool critical_zone = state.dumping_active || state.pending_bucket_secured || (state.empty_confirm_frames > 0);

        if (critical_zone && main_truck_rect.area() > 0) {
            float frame_area = (float)(frame.cols * frame.rows);
            float max_mine_area = 0.0f;

            for (const auto& m : mine_boxes) {
                cv::Rect m_rect(m.xmin, m.ymin, m.xmax - m.xmin, m.ymax - m.ymin);
                if (check_horizontal_overlap(main_truck_rect, m_rect)) {
            ...
            state.ratio_buffer.push_back(3.0f * max_mine_area / frame_area);

            // 防止长时间停滞导致内存溢出
            if (state.ratio_buffer.size() > 100) {
                state.ratio_buffer.erase(state.ratio_buffer.begin());
            }
        }

        // ============================== E. 快速强行合并兜底 ==============================
        if (state.pending_buckets > 0 && state.frames_since_bucket_empty > 60 && !state.dumping_active) {
            if (state.last_avg_ratio < 0.0f) state.last_avg_ratio = 0.0f;
            commit_pending_buckets();
        }
